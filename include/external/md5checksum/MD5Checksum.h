...
The URL given in the license text has been reused for other purposes.
The original content can be viewed using Wayback Machine:
https://web.archive.org/web/20020810085540/http://www.langfine.com/
*/
...
*****************************************************************************************/
  ...
  */
  ...
  NOTES:      Calculates the RSA MD5 checksum for a file or contiguous array of data.
...
namespace agxStream {
  ...
  class AGXPHYSICS_EXPORT MD5Checksum
  {
  public:
    /**
    Read numBytes from \p pBuf and create a MD5 checksum from the read data.
    Will Throw std::runtime_error if error occurs.
    \return string with MD5 checksum based on in-data.
    */
    static std::string getMD5(const char* pBuf, size_t numBytes);

    /**
    Read from \p stream until end of file create a MD5 checksum from the read data.
    Will Throw std::runtime_error if error occurs.
    \return string with MD5 checksum based on in-data.
    */
    static std::string getMD5(std::istream& stream);

    /**
    Read until end of file in file \p strFilePath and create a MD5 checksum from the read data.
    Will Throw std::runtime_error if error occurs.
    \return string with MD5 checksum based on in-data.
    */
    ...
    /**
    Read \p numBytes (or until end of stream) from \p stream and create a MD5 checksum from the read data.
    Will Throw std::runtime_error if error occurs.
    \return string with MD5 checksum based on in-data.
    */
    static std::string getMD5(std::istream& stream, size_t numBytes);
  ...
  protected:
    ...
    void update(const char* Input, size_t nInputLen);
    std::string final();
    ...
    //utility functions
    inline void dWordToByte(char* Output, uint32_t* Input, uint32_t nLength);
    inline void byteToDWord(uint32_t* Output, const char* Input, uint32_t nLength);
