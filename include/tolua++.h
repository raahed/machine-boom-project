...
#define TOLUA_VERSION "tolua++-1.0.92-agx1"
...
// Ref +1 in AGX type of class Referenced.
template< typename T >
void TOLUA_REFERENCED_CLASS_ref( T* p )
{
  p->reference();
}
...
// Ref -1 in AGX type of class Referenced.
template< typename T >
void TOLUA_REFERENCED_CLASS_unref( T* p )
{
  if ( p->getReferenceCount() )
    p->unreference();
}

// Ref +1 in OSG type of class Referenced.
template< typename T >
void TOLUA_REFERENCED_CLASS_ALT1_ref( T* p )
{
  p->ref();
}
...
// Ref -1 in OSG type of class Referenced.
template< typename T >
void TOLUA_REFERENCED_CLASS_ALT1_unref( T* p )
{
  if ( p->referenceCount() )
    p->unref();
}
