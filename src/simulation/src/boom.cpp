...
Joint *const Boom::getJoint(const std::string &name) {
    ...
    errorStream << "The name: " << name
                << " is not a valid joint on this boom please check the name again and verify its part of the boom!";
    throw std::invalid_argument(errorStream.str());
}

const std::vector<double> Boom::getLowestCableNode() {
    std::vector<double> lowestNode{std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity(),
...
Boom::Builder *Boom::Builder::addJoint(agx::Name &name) {
    static_assert(std::is_base_of<agx::Constraint, T>::value,
                  "The type parameter of Boom::Builder::addJoint() must derive from agx::Constraint");
...
Boom *Boom::Builder::build() {
    if (!isInitialized()) {
        throw std::invalid_argument(
                "Please provide a boom with cables and joints and make sure they exist in the simulation before building a Boom object!");
    }
